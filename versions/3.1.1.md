...
* keywords "MUST", "MUST NOT", "REQUIRED", "SHALL", "SHALL NOT", "SHOULD", "SHOULD NOT", "RECOMMENDED", "NOT RECOMMENDED", "MAY", and "OPTIONAL" follow
...
* == template expressions (`{}`) / used | URL path
  * MUST correspond -- to a -- path parameter 
    * | 
      * [Path Item](#path-item-object) itself &/OR
      * EACH Path Item's [Operations](#operation-object)
    * /
      * ❌'s values MUST NOT contain any unescaped "generic syntax" characters❌ / described by [RFC3986](https://tools.ietf.org/html/rfc3986#section-3)
        * forward slashes (`/`),
        * question marks (`?`),
        * hashes (`#`)

* if the path item is empty -> get an exception
  * _Example:_ due to ACL constraints -> match path parameters are NOT required
...
* ALLOWED values
  * `path`
    * uses
      * \+ [Path Templating](#path-templating) | parameter value == part of the operation's URL
    * != API's host OR base path 
    * _Example:_ `/items/{itemId}` -> path parameter: `itemId`
  * `query`
    * == parameters / appended | URL
      * _Example:_ `/items?id=###` -> query parameter: `id`
  * `header`
    * == custom headers / 
      * uses
        * part of the request
      * ' names are case insensitive
        * Reason: 🧠follow [RFC7230](https://tools.ietf.org/html/rfc7230#section-3.2)🧠 
  * `cookie`
    * uses
      * pass a SPECIFIC cookie value | API
...
* Parameter Objects
  * MUST include
    * `content` field OR
    * `schema` field
* see [Appendix B](#appendix-b-data-type-conversion)
...
| Field Name | Type | Description |
| ---- | :----: | ---- |
| <a name="parameter-name"></a>name | `string` | ⚠️**REQUIRED**⚠️ <br/> == 👀parameter's name👀 <br/>  _case sensitive_ <ul><li>if [`in =="path"`](#parameter-in) -> `name` field MUST correspond -- to a -- template expression / occurr \| [Paths Object](#paths-object)'s [path](#paths-path) field <li>if [`in == "header"`](#parameter-in) & `name` == `"Accept"`, `"Content-Type"` OR `"Authorization"` -> parameter definition SHALL be ignored <li> REST of cases, `name` correspond -- to the -- parameter name used \| [`in`](#parameter-in) field </ul> |
| <a name="parameter-in"></a>in | `string` | ⚠️**REQUIRED**⚠️ <br/> == parameter's location <br/> ALLOWED values <br/> &nbsp;&nbsp; `"query"` <br/> &nbsp;&nbsp; `"header"` <br/> &nbsp;&nbsp; `"path"` <br/> &nbsp;&nbsp; `"cookie"` |
| <a name="parameter-description"></a>description | `string` | == parameter's brief description <br/> it could contain examples <br/> if you want to enrich the text -> use [CommonMark syntax](https://spec.commonmark.org/) |
| <a name="parameter-required"></a>required | `boolean` | by default, `false` <br/> ⚠️if the [parameter location](#parameter-in) == `"path"` -> MUST be `true`⚠️ |
| <a name="parameter-deprecated"></a> deprecated | `boolean` | by default, `false` <br/> == parameter's deprecation |
| <a name="parameter-allow-empty-value"></a> allowEmptyValue | `boolean` | by default, `false` <br/> if `true` -> clients MAY pass a string / 's value's length == 0 <br/> &nbsp;&nbsp; server SHOULD interpret -- as -- parameter unused <br/>  if [`style`](#parameter-style) is used & [behavior == _n/a_ (== can NOT be serialized)](#style-examples) -> `allowEmptyValue`'s value SHALL be ignored <br/> interactions between this field -- & -- parameter's [Schema Object](#schema-object) are implementation-defined <br/> uses <br/> &nbsp;&nbsp; ONLY \| `query` parameters <br/> &nbsp;&nbsp; ❌NOT RECOMMENDED❌ <br/> &nbsp;&nbsp;&nbsp;&nbsp; Reason:🧠likely to be removed later🧠 |

* if you want to extend this -> use [Specification Extensions](#specification-extensions)

* if `in == "header"` -> `name` (can be) == `"Cookie"`
  * ❌NOT RECOMMENDED❌
    * Reason: 🧠NOT proper way -- to -- define a cookie parameter🧠
    * 👀refactor -- as -- `in: "cookie"`👀

###### Fixed Fields / used | `schema`
...
Serializing typed data to plain text, which can occur in `text/plain` message bodies or `multipart` parts, as well as in the `application/x-www-form-urlencoded` format in either URL query strings or message bodies, involves significant implementation- or application-defined behavior.
 ...
 for a discussion of converting values of various types to string representations.
